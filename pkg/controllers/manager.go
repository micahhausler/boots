...
const (
	HardwareMacAddrKey  = "spec.interfaces.dhcp.mac"
	HardwareIPAddrKey   = "spec.interfaces.dhcp.ip.address"
	HardwareHostnameKey = "spec.interfaces.dhcp.hostname"
	WorkflowWorkerKey   = "status.tasks.workeraddr"
)
...
func NewHardwareManagerOrDie(config *rest.Config, options controllerruntime.Options) Manager {
	manager, err := NewHardwareManager(config, options)
	if err != nil {
		panic(err)
	}
	return manager
...
func NewHardwareManager(config *rest.Config, options controllerruntime.Options) (Manager, error) {
...
func macIndexFunc(obj client.Object) []string {
	wf, ok := obj.(*v1alpha1.Hardware)
	if !ok {
		return nil
	}
	resp := []string{}
	for _, iface := range wf.Spec.Interfaces {
		if iface.DHCP != nil {
...
func ipIndexFunc(obj client.Object) []string {
	wf, ok := obj.(*v1alpha1.Hardware)
	if !ok {
		return nil
	}
	resp := []string{}
	for _, iface := range wf.Spec.Interfaces {
		if iface.DHCP != nil && iface.DHCP.IP != nil {
...
func hostnameIndexFunc(obj client.Object) []string {
	wf, ok := obj.(*v1alpha1.Hardware)
	if !ok {
		return nil
	}
	...
	for _, iface := range wf.Spec.Interfaces {
		if iface.DHCP != nil {
			resp = append(resp, iface.DHCP.Hostname)
		}
	}
	return resp
}
